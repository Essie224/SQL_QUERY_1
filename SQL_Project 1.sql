...
--INSERT VALUES INTO PRODUCTS VALUE
...
INSERT INTO PRODUCTS VALUES (22, 'Gustaf Kn�ckebr�d', 9, 5, '24 cartons (500 g)', 105.00, 104, 0, 0);
...
SELECT EMPLOYEE_NUMBER, FIRST_NAME, LAST_NAME, 
       ...
       TITLE,
       COALESCE(SALARY, 0) + COALESCE(COMMISSION, 0) AS net_salary
FROM EMPLOYEES where title IN ('Dr.', 'Mr.')
and COALESCE(SALARY, 0) + COALESCE(COMMISSION, 0) >= 8000 
...
SELECT PRODUCT_REF, PRODUCT_NAME, SUPPLIER_NUMBER, UNITS_ON_ORDER, UNIT_PRICE, QUANTITY
FROM PRODUCTS
WHERE QUANTITY LIKE '%bottle%' OR QUANTITY like '%bottles%'
AND PRODUCT_NAME LIKE '%___t%' OR PRODUCT_NAME LIKE '%___T%'
...
--and the last three digits of the postal code. The query should utilize a single subquery. The resulting
--table should include all columns from the customer table.
...
--is between 0 and 2000, 5% if between 2001 and 10000, 10% if between 10001 and 40000, 15% if between 40001
--and 80000, and 20% otherwise.-Display the message "apply old discount rate" if the order number is between 10000 and 10999, and
--"apply new discount rate" otherwise. The resulting table should display the columns: order number, new discount rate,
--and discount rate application note.

SELECT * FROM ORDER_DETAILS
SELECT * FROM ORDERS

SELECT order_number, sum(unit_price * quantity) as Total,
CASE
...
ELSE 20 END AS New_Discount_Rate,
CASE
WHEN ORDER_NUMBER BETWEEN 10000 AND 10999 THEN 'Apply_Old_Discount_Rate'
ELSE 'Apply_New_Discount_Rate' END AS Discount_Rate_Application_Note
FROM ORDER_DETAILS
...
SELECT S.SUPPLIER_NUMBER, S.COMPANY, S.ADDRESS, S.PHONE, P.PRODUCT_NAME, C.CATEGORY_NAME  
FROM SUPPLIERS AS S
...
SELECT DISTINCT C.CUSTOMER_CODE, C.CITY,O.ORDER_NUMBER, P.PRODUCT_NAME, CA.CATEGORY_NAME,OD.QUANTITY FROM CUSTOMERS C
...
WHERE  C.CITY = 'BERLIN' 
GROUP BY C.CUSTOMER_CODE, C.CITY,O.ORDER_NUMBER, P.PRODUCT_NAME, CA.CATEGORY_NAME,OD.QUANTITY
HAVING COUNT(CASE WHEN CA.CATEGORY_NAME = 'Desserts' THEN 1 ELSE NULL END) <= 1
...
LEFT JOIN 
    ...
    AND DATEPART(DW FROM O.ORDER_DATE) = 2  
    AND DATEPART(MONTH FROM O.ORDER_DATE) = 4  
    AND DATEPART(YEAR FROM O.ORDER_DATE) = 1998
LEFT JOIN 
...
ORDER BY 
 ...
 --Display customers who have ordered all products. The resulting table should display the columns: customer code,
 --company name, and telephone number.
 
SELECT 
    C.CUSTOMER_CODE,
    C.COMPANY AS 'Company_Name',
    C.Phone AS 'Telephone_Number'
FROM 
    CUSTOMERS C
WHERE 
    NOT EXISTS (
...
--Display for each customer from France the number of orders they have placed. The resulting table should display the 
...
WHERE 
    C.COUNTRY = 'France'
GROUP BY 
    C.CUSTOMER_CODE,COUNTRY
ORDER BY C.CUSTOMER_CODE;

--Display the number of orders placed in 1996, the number of orders placed in 1997, and the difference between these 
--two numbers. The resulting table should display the columns: orders in 1996, orders in 1997, and Difference.

SELECT 
  SUM(CASE WHEN DATEPART(YEAR FROM O.ORDER_DATE) = 1996 THEN OD.QUANTITY ELSE 0 END) AS 'QUANTITY PURCHASED IN 1996',
  SUM(CASE WHEN DATEPART(YEAR FROM O.ORDER_DATE) = 1997 THEN OD.QUANTITY ELSE 0 END) AS 'QUANTITY PURCHASED IN 1997',
  SUM(CASE WHEN DATEPART(YEAR FROM O.ORDER_DATE) = 1996 THEN OD.QUANTITY ELSE 0 END) - 
  SUM(CASE WHEN DATEPART(YEAR FROM O.ORDER_DATE) = 1997 THEN OD.QUANTITY ELSE 0 END) AS 'DIFFERENCE'
FROM ORDERS O
	JOIN ORDER_DETAILS OD ON O.ORDER_NUMBER = OD.ORDER_NUMBER
WHERE DATEPART(YEAR FROM O.ORDER_DATE) IN (1996, 1997);
...
SELECT * FROM PRODUCTS  
 ...
 WITH TABLE1 AS (SELECT CUSTOMER_CODE,COMPANY,PHONE FROM CUSTOMERS)--DISTINCT PRODUCT_REF, COUNT(*) FROM PRODUCTS
